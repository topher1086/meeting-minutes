...
# Color codes - these are the same on macOS and Ubuntu
GREEN='\033[0;32m'
BLUE='\033[0;34m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
...
# Helper functions for logging - these are the same
log_info() {
...
# Error handling - this is the same
handle_error() {
    log_error "$1"
    exit 1
}
...
sudo apt-get install -y libomp-dev llvm cmake || handle_error "Failed to install dependencies"


log_info "Setting up compiler environment..."
# Ubuntu uses /usr/bin for clang by default, and doesn't need explicit LDFLAGS/CPPFLAGS
# Check if clang is installed, and get the version.  Use this for both CC and CXX.
if ! clang --version > /dev/null 2>&1; then
  handle_error "Clang compiler not found. Please install it."
fi
...
# No need to specify compilers if they are in the standard path
cmake .. || handle_error "CMake configuration failed"
make -j$(nproc) || handle_error "Make failed"  # Use nproc for available cores
cd ..
...
# Create necessary directories - this is the same
...
# Copy server binary - this is the same
...
# Check for existing models - this is the same
...
# Create run script - this is the same
...
if [ -d "../$PACKAGE_NAME" ]; then
    ...
    # rm -r "$PACKAGE_NAME"  # Consider if you want to remove the original
fi
